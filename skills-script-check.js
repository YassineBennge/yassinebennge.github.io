    ...
    const ANIMATION_CLASSES = ["in", "revealed", "animated", "visible", "in-view"];
    ...
    function buildSkills(){
      const allSkills = [...new Set([
        ...resumeData.skills.map(s=>s.key),
        ...resumeData.technologies
      ])];
      const midpoint = Math.ceil(allSkills.length / 2);
      appendSkillTrack(document.getElementById("skillTrackA"), allSkills.slice(0, midpoint));
      appendSkillTrack(document.getElementById("skillTrackB"), allSkills.slice(midpoint));
    }

    function emojiForSkill(name){
      const s = name.toLowerCase();
      if (s.includes("python")) return "\u{1F40D}";
      if (s.includes("react")) return "\u269B\uFE0F";
      if (s.includes("node")) return "\u{1F7E2}";
      if (s.includes("mongo") || s.includes("mysql") || s.includes("data")) return "\u{1F5C4}\uFE0F";
      if (s.includes("machine") || s.includes("deep") || s.includes("vision") || s.includes("intelligence")) return "\u{1F9E0}";
      if (s.includes("git")) return "\u{1F527}";
      if (s.includes("design") || s.includes("photoshop") || s.includes("illustrator")) return "\u{1F3A8}";
      if (s.includes("premiere") || s.includes("after") || s.includes("youtube") || s.includes("tiktok")) return "\u{1F3AC}";
      if (s.includes("api") || s.includes("jwt")) return "\u{1F510}";
      if (s.includes("html") || s.includes("css") || s.includes("javascript")) return "\u{1F4BB}";
      return "\u2728";
    }

    function appendSkillTrack(track, skills){
      if (!track) return;
      const loop = [...skills, ...skills];
      loop.forEach((skill, i)=>{
        const chip = document.createElement("span");
        chip.className = "marqueeSkillChip accent-" + (i % 4);
        chip.textContent = emojiForSkill(skill) + " " + skill;
